(ns pacer_courts.main
  "Kotodama WASM entrypoint for the pacer_courts clean-room actor (L4) — Clojure port.
  ...
  facts (`pacer_courts.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "pacer_courts")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "pacercou_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "pacercou_par"
    :fields [:caseId :role :name]           :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "pacercou_doc"
    :fields [:caseId :type :contentRef]     :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "pacercou_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "pacercou_wat"
    :fields [:name :program :entries]       :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "pacercou_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"pacer_courts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "pacer_courts-compat" :tier tier :entities entities} 200])
...
(defn start! [] :pacer_courts-compat/ready)
